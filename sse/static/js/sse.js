...
function retrieveData(response, Kenc, searchNo, searchNoUri, keyword) {
  ...
  if (response == undefined) {
    //|| response.Cfw.length==0 ){ //found 0 results
    found = 0;
  ...
  if (msg != "error") {
    ...
    if (searchNo == 1) {
      ...
      var jsonData =
        '{ "w" : "' + hash(keyword) + '","searchno" : ' + searchNo + "}";
...
function updateFileNo(Lhash, LfileNoUri, LfileNo, offset) {
  ...
  for (i = 0; i < length; i++) {
    ...
    if (new_fileno <= 0) {
      ...
      //&& fileno+offset>0, Addition: if after update, there exists file conatining the keyword
      update = true;
...
function updateData(data, file_id, sharedKey, Kenc, callback) {
  ...
  Lcurrent_fileNoUri = [];
  current_listW = [];
  [Lcurrent_fileNo, Lcurrent_fileNoUri, current_listW] = getMultiFileNo(
    Lcurrent_hash
  );
  ...
  [Lall_found_searchNo, Lall_searchNoUri, all_tempListWord] = getMultiSearchNo(
    Lall_hash
  );
  ...
  if (result.status == HTTP_CODE_CREATED) {
    ...
    [
      ...
      current_deleted_objects,
      delete_current_searchno,
    ] = updateFileNo(Lcurrent_hash, Lcurrent_fileNoUri, Lcurrent_fileNo, -1);
    ...
    [
      ...
      new_deleted_objects,
      delete_new_searchno,
    ] = updateFileNo(Lnew_hash, Lnew_fileNoUri, Lnew_fileNo, 1);
...
function deleteData(file_id, sharedKey, Kenc, callback) {
  ...
  var obj = getRequest(
    sseConfig.base_url_sse_server + "/api/v1/ciphertext/?jsonId=" + file_id
  );
  console.log("response:", obj);
  ...
  if (length == 0) {
    ...
    var objMap = getRequest(
      sseConfig.base_url_sse_server + "/api/v1/map/?value=" + file_id
    );
